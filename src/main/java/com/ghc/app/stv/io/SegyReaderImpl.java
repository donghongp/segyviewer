...
import com.ghc.jtk.io.ArrayFile;
import com.ghc.jtk.io.Check;

/**
 * A uniformly sampled seismic image in SEG-Y format. The image may be two- or
 * three-dimensional. The 1st dimension is assumed to be time (or depth), and the 2nd
 * dimension corresponds to the inline direction. For 3D images the 3rd dimension
 * corresponds to the crossline direction.
 * <p>
 * All time and space coordinates are converted (if necessary) and stored as seconds and
 * kilometers.
 * <p>
 * The image sampling grid has three indices i1, i2, and i3 in ranges [0,n1-1],
 * [i2min,i2max], and [i3min,i3max], corresponding to time, inline, and crossline
 * dimensions, respectively. The minimum inline and crossline indices i2min and i3min in
 * SEG-Y trace headers are typically non-zero.
 * <p>
 * Image traces may be obtained from the image by specifying either a sequential trace
 * index itrace or a pair of grid indices (i2,i3). For grid indices at which a trace does
 * not exist, sample values are set to zero.
 *
 * @author Dave Hale, Colorado School of Mines
 * @version 2012.06.18
 */
public class SegyReaderImpl {
	 ...
	 */
	public SegyReaderImpl(String fileName) {
	 ...
	 */
	public SegyReaderImpl(String fileName, ByteOrder byteOrder) {
		_fileName = fileName;
	...
	/**
	 * Prints summary information derived from the binary file header. To ensure that this
	 * information makes sense, this method is often the first one called after
	 * constructing an SEG-Y image. Nonsensical information may be caused by non-standard
	 * byte order or non-standard SEG-Y headers.
	 * <p>
	 * For SEG-Y files with either IBM or IEEE floats, this method reads a small number of
	 * traces to guess the format code (because this code is often set incorrectly in the
	 * binary file header), and prints a warning if the guess does not match the format in
	 * the header.
	 */
	...
	/**
	 * Prints all information derived from binary file and trace headers. This method
	 * reads the entire SEG-Y file, which may take a long time.
	 */
	...
	/**
	 * Prints the binary file header. Useful for diagnosing problems. See the SEG-Y
	 * standard for complete explanations of all fields.
	 */
	...
	/**
	 * Prints a specified trace header. Useful for diagnosing problems. See the SEG-Y
	 * standard for complete explanations of all fields.
	 * 
	...
	public int[] getFileBinaryHeaderValue() {
		loadBinaryHeaderInfo();
		int[] h = new int[400 / 4];
		try {
			_af.seek(3200);
		...
		int[] headerValues = new int[_binaryHeaderFields.length];
		int k = 0;
		for (String field : _binaryHeaderFields) {
			int ibyte = Integer.parseInt(field.substring(0, 4));
			int nbyte = Integer.parseInt(field.substring(5, 9)) - ibyte + 1;
	...
	public int[] getTraceHeaderValue(int i) {
		loadBinaryHeaderInfo();
		int[] h = new int[240 / 4];
		try {
			_af.seek(headerOffset(i));
		...
		int[] headerValues = new int[_traceHeaderFields.length];
		int k = 0;
		for (String field : _traceHeaderFields) {
			int ibyte = Integer.parseInt(field.substring(0, 4).trim());
			int nbyte = Integer.parseInt(field.substring(5, 9).trim()) - ibyte + 1;
	...
	/**
	 * Gets the format code for this image. The code is 1 for IBM floats, 5 for IEEE
	 * floats.
	 * 
	...
	/**
	 * Sets the format code for this image. Overrides the format found in the binary file
	 * header.
	 * 
	...
	/**
	 * Sets bytes used to find inline and xline numbers in trace headers. Both inline and
	 * xline numbers are represented by four-byte integers, but these integers are not
	 * always placed in the standard bytes 189-192 (for inline) and 193-196 (for xline) of
	 * trace headers. This method allows these standard byte locations to be overridden.
	 * <p>
	 * Note that the first byte is 1, not 0, as in the documentation for SEG-Y format.
	 * 
	...
	/**
	 * Sets the sampling interval for the 1st dimension. Overrides the interval found in
	...
	/**
	 * Sets the sampling interval for the 2nd dimension. Overrides the interval computed
	 * from trace headers.
	 * 
	...
	/**
	 * Sets the sampling interval for the 3rd dimension. Overrides the interval computed
	 * from trace headers.
	 * 
	...
	/**
	 * Gets the trace with specified grid indices. If a trace does not exist for those
	 * indices, all samples are zero.
	 * 
	...
	public String readEBCDIC() {
		try {
			int nlines = 40;
			StringBuilder sb = new StringBuilder();
			int ncols = 80;
			byte[] ebcdic = new byte[ncols];
			_af.seek(0);
			for (int i = 0; i < nlines; i++) {
				_af.read(ebcdic);
				sb.append(new String(ebcdic, "Cp1047"));
	...
	public String readEBCDICByte() {
		try {
			byte[] ebcdic = new byte[3200];
			_af.seek(0);
			_af.read(ebcdic);
			return new String(ebcdic, "Cp1047");
		} catch (IOException e) {
			return null;
		}
	}
	...
	public static String[] _binaryHeaderFields = {
	...
	public static String[] _traceHeaderFields = {
	...
	private void loadTraceHeaderInfo() {
		...
		for (int j3 = _i3min; j3 <= _i3max; ++j3) {
			...
			if (j2hi - j2lo > k2) {
				...
				if (dd > 0.0) {
					if (!_d2Set)
						_d2 = Math.sqrt(dx * dx + dy * dy) / k2;
					double az = Math.atan2(dx, dy);
					_sin2 = Math.sin(az);
					_cos2 = Math.cos(az);
					_azim = Math.toDegrees(az);
				}
			}
		}
		int k3 = 0;
